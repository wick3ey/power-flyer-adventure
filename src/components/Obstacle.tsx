...
const ObstacleComponent: React.FC<ObstacleProps> = ({ obstacle }) => {
  ...
  return (
    ...
    >
      ...
      {isPipe && (
        <div className={`absolute -top-4 -left-2 -right-2 h-6 
                        ${obstacle.y > 200 ? '-top-4' : 'top-full -mt-2'} 
                        bg-gradient-to-b from-green-500 to-green-700 
                        border-2 border-green-900 rounded-md`}>
          <div className="absolute inset-1 bg-gradient-to-b from-transparent to-green-900/20 rounded-sm"></div>
        </div>
      )}
      
      {/* Inner shadow for depth */}
      <div className="absolute inset-2 rounded-sm bg-gradient-to-b from-transparent to-green-900/30"></div>
    </motion.div>
  );
};
